...
if [ -n "$PIDS" ] ; then
    ...
    java -Xmx1024m  -Ddk.netarkivet.settings.file={{dir}}/conf/settings_{{app}}.xml -Dlogback.configurationFile={{dir}}/conf/logback_{{app}}.xml dk.netarkivet.harvester.heritrix3.{{app_name}} < /dev/null > start_{{app}}.log 2>&1 &
fi